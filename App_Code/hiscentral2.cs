...
public class hiscentral2 : System.Web.Services.WebService {
    ...
    public Site[] GetSitesInBox(Box box, string conceptKeyword, int[] networkIDs) {
        ...
        if (conceptKeyword != null && conceptKeyword != "") {
            ...
            string rootID = getConceptID(conceptKeyword); // can be empty
            if (conceptids == "" && rootID!=null) conceptids = rootID.ToString();
            else if (conceptids != "" && rootID!=null) conceptids = conceptids + "," + rootID.ToString(); 
            if (conceptids != "" || rootID != null) 
                sql += " AND (conceptid in (" + conceptids + "))";
            //conceptCode = getCommaString(getChildConceptsFlat(conceptCode));
            // if (!(conceptids == "")) {  // this will never happen, because the root concept should be included.  
                // this must contain the conceptid of the parent, as well as the children. 
                // sql += " AND conceptid in (" + conceptids + ")";
            // }
...
Networkids == null ? string.Empty : String.Join(",", Networkids))
        ...
        string sql = "SELECT     MappingsApproved.ConceptID, Variables.AltVariableName, Variables.AltVariableCode, Variables.ValueType, Variables.IsRegular, " +
                      ...
                      "FROM HISNetworks WITH (NOLOCK) INNER JOIN " +
                         "Variables WITH (NOLOCK) ON HISNetworks.NetworkID = Variables.networkId INNER JOIN " +
                         "MappingsApproved WITH (NOLOCK) ON Variables.VariableID = MappingsApproved.VariableID " +
...
ServiceStatus
  ...
  ,ConceptKeyword ,ConceptID ,ValueType        */
    ...
    private string getConceptID(string conceptKeyword)
    {
    ...
    public SeriesRecord[] getSeriesCatalogInBoxPaged(double xmin, double xmax, double ymin, double ymax, string conceptKeyword, String networkIDs, string beginDate, string endDate, int pageno) {
        ...
        queryLog.InfoFormat(queryLogFormat, methodName, "Start", 0,
           String.Format(objecformat,
           conceptKeyword ?? String.Empty,
           xmin, xmax, ymin, ymax, networksString,
           beginDate ?? String.Empty, endDate ?? String.Empty)
           );
        ...
        using (con) {
            ...
            if (filterNetowrk && filterKeyword) cmd.CommandText = "sp_couch_new_SeriesSearch_keyword_NetworkIDs";
    ...
    public int[] getChildIDsFlat(String conceptName) {
        ...
        using (con) {
            ...
            if (rowcount > 0) {
                ...
                concepts = new int[1];
                concepts[0] = id;
                sql = "Select conceptid from conceptPaths WITH (NOLOCK) where path like '" + path + conceptName + "/%'";
                da = new SqlDataAdapter(sql, con);
                da.Fill(ds, "conceptids");
                int i = 0;
                rowcount = ds.Tables["conceptids"].Rows.Count;
    ...
    public String[] getChildConceptsFlat(String conceptName) {
        ...
        concepts[0] = conceptName;
        String sql = "Select path from conceptPaths WITH (NOLOCK) where conceptName = @conceptName";
        ...
        using (con) {
            ...
            if (rowcount > 0) {
                ...
                sql = "Select conceptName from conceptPaths WITH (NOLOCK) where path like '" + path + "%'";
                da = new SqlDataAdapter(sql, con);
                da.Fill(ds, "concepts");
                int i = 0;
